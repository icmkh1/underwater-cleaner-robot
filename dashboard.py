...
class Dashboard(QWidget):
    ...
    def _build_magnet(self):
        ...
        mrow.addStretch(1)
        # 磁力单位：PWM（与推进器输出与告警面板的 PWM 同一水平线）
        pwm_unit = QLabel("PWM")
    ...
    def _set_mag_status(self, level):
        """level: excellent(优良) / normal(正常) / warn(建议手动介入)"""
        styles = {
            "excellent": ("优良", GREEN),
            "normal": ("正常", CYAN),
            "warn": ("建议手动介入", YELLOW),
    ...
    def _set_mag_mode(self, mode):
        ...
        if mode == "auto":
            # 自主模式：只读显示主控模拟目标磁力（待接入真实自主算法），并写入共享状态
